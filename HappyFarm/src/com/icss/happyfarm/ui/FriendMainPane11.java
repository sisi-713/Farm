 ...
 */
public class FriendMainPane11 extends JPanel {
   ...
   // private int lastRecord;       //当前页的结束记录
    ...
    public FriendMainPane11(User user) {
    ...
    public void initPane(int currentPage) {        //数据分页显示，配合right哈left标签
        ...
        int m = 0, i = 0;
        try {
            rs.absolute((currentPage-1)*6);
            while (rs.next()) {
                String str = rs.getString("userPicture");
                String str2 = rs.getString("userName");
               if(i<6){
                //  System.out.println(str+"\t"+str2);
               // fp[m].getLblFriend()[1].setIcon(new ImageIcon(getClass().getResource(url + "number/nub" + j + ".png")));
...
//        selectField.transferFocus();
    ...
    public void search() {
        ...
        if (text.equals("") || text.equals("按昵称查找")) {
            ...
            sch = true;
            currentPage = 1;
            FriendMainPane11.this.initDataBase3(text);
            FriendMainPane11.this.initPane(currentPage);
        }
    }
            ...
            if (lblTemp.getName().equals("left")) {
                ...
                if (currentPage <= 1) {
                    ...
                    if (sch) {
                        ...
                        if (flag) {
                            FriendMainPane11.this.initDataBase();   //必须执行此函数，否则将丢失数据，悲剧 的游标遍历。
                        } else {
                            FriendMainPane11.this.initDataBase2();
                        }
                    }
                    FriendMainPane11.this.initPane(currentPage);
                    pageLabel.setText(currentPage + "/" + howManyPage);
...
//                    System.out.println("不能往右边翻了");
                    ...
                    if (flag) {
                        FriendMainPane11.this.initDataBase();
                    } else {
                        FriendMainPane11.this.initDataBase2();
                    }
                    }
...
//                new addFrame();
                ...
                int x = FriendMainPane11.this.getX();
                int y = FriendMainPane11.this.getY();
        ...
        public void run() {
            ...
            try {
                if (x == 971 || x == 970) {
                    for (x = 972; x > 820; x -= 2) {
                        Thread.sleep(1);
                        FriendMainPane11.this.setLocation(x, y);
