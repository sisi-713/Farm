...
public class RoomTypeController {
	...
	public Map<String, String> add(RoomType roomType){
		...
		if(roomType == null){
			ret.put("type", "error");
			ret.put("msg", "请填写正确的土地类型信息!");
			return ret;
		}
		if(StringUtils.isEmpty(roomType.getName())){
			ret.put("type", "error");
			ret.put("msg", "土地类型名称不能为空!");
			return ret;
		}
		roomType.setAvilableNum(roomType.getRoomNum());//默认土地数等于可用土地数
	...
	public Map<String, String> edit(RoomType roomType){
		...
		if(roomType.getAvilableNum() <= 0){
			roomType.setAvilableNum(0);//没有可用土地
	...
	public Map<String, String> delete(Long id){
		...
		try {
			...
			ret.put("type", "error");
			ret.put("msg", "该土地类型下存在土地信息，请先删除该土地类型下的所有土地信息!");
			return ret;
		}
		ret.put("type", "success");
		ret.put("msg", "删除成功!");
